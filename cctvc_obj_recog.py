...
def detectedObjectResponse():
    ...
    #print(detectedObjectResponse_isRunning)
...
#   the stream with cv2.
def opencamerastream(ADDRESS, USERNAME, PASSWORD, CHANNELSELECT, SMD):
    global detectedObjectResponse_isRunning
...
#   RTSP Stream
        if event == 'Live View':
            ...
            if ping(ADDRESS) == True:
                ...
                print("response code:\n"+str(response.status_code))
                if response.status_code == 200:
                    camstream = multiprocessing.Process(target=opencamerastream, args=(ADDRESS,USERNAME,PASSWORD,CHANNELSELECT,SMD))
